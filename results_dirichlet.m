...
[us{1,1},x{1},y{1},t]= solve_st(u0,rg,T,area,h(1),k,n(1),'Dirichlet');
[us{2,1},x{2},y{2}]  = solve_st(u0,rg,T,area,h(2),k,n(1),'Dirichlet');
[us{1,2}]            = solve_st(u0,rg,T,area,h(1),k,n(2),'Dirichlet');
[us{2,2}]            = solve_st(u0,rg,T,area,h(2),k,n(2),'Dirichlet');
[us{1,3}]            = solve_st(u0,rg,T,area,h(1),k,n(3),'Dirichlet');
[us{2,3}]            = solve_st(u0,rg,T,area,h(2),k,n(3),'Dirichlet');

tf = cell(2,1); tc = cell(2,1);
xf = cell(2,1); xc = cell(2,1);
yf = cell(2,1); yc = cell(2,1);
[ uf{1}, tf{1}, xf{1}, yf{1} ] = ...
    solve_ftcs(h(1),lambda,u0.fun,[0,T],area(1,:),area(2,:),'Dirichlet');
[ uf{2}, tf{2}, xf{2}, yf{2} ] = ...
    solve_ftcs(h(2),lambda,u0.fun,[0,T],area(1,:),area(2,:),'Dirichlet');
[ uc{1}, tc{1}, xc{1}, yc{1} ] = ...
    solve_cn(h(1),lambda,u0.fun,[0,T],area(1,:),area(2,:),'Dirichlet');
[ uc{2}, tc{2}, xc{2}, yc{2} ] = ...
    solve_cn(h(2),lambda,u0.fun,[0,T],area(1,:),area(2,:),'Dirichlet');
...
for j=1:2
    for l=1:length(terr)
        for i=1:3
            us_max(j,i,l) = max(reshape(us{j,i}(:,:,t==terr(l)),1,[]));
        end
        uf_max(j,l) = max(reshape(uf{j}(:,:,tf{j}==terr(l)),1,[]));
        uc_max(j,l) = max(reshape(uc{j}(:,:,tc{j}==terr(l)),1,[]));
    end
end
